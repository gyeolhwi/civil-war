...
//     · 티어·등급 화면: 한 역할의 티어+등급을 함께. 위 역할 탭(탱/딜/힐)으로 역할 전환.
//       3역할×(티어+등급)=6셀렉트가 5행을 넘으므로 역할 단위로 본다(어느 역할인지 제목·탭).
//
...
// 같은 배포(서버리스 warm 인스턴스)에서 반복 클릭 시 채널/멤버 해석 쿼리를 줄이는
// 모듈 캐시. guild_id·discord_user_id 매핑은 거의 안 바뀌어 안전하다. (지연 개선)
...
export async function resolveChannelId(
  ...
  const cached = channelIdCache.get(guildId);
  ...
  const { data } = await sb
    ...
    .maybeSingle();
  if (data) {
    channelIdCache.set(guildId, data.id as string);
  ...
  if (all && all.length === 1) {
    await sb
      ...
      .eq("id", all[0].id);
    channelIdCache.set(guildId, all[0].id as string);
...
/** 메인 프로필 화면 — 닉네임·배틀태그·현황 요약 + 주/부 포지션 셀렉트 + 작성 버튼. */
...
/** 티어·등급 화면의 역할 탭 — 현재 편집 중인 역할은 초록+✅. */
...
/**
 * 티어·등급 화면 — 한 역할의 티어+등급을 함께(분리 X). 위 탭으로 역할 전환.
 * 어느 역할인지 제목·탭·셀렉트 라벨로 분명히 한다. (탭은 같은 화면 내 전환)
 * 역할 탭(1) + 티어(1) + 등급(1) + 뒤로(1) = 4행. 3역할 6셀렉트가 5행을 넘어
 * 한 화면에 못 담기 때문에 역할 단위로 본다.
 */
function tierResponse(
  ...
  const head = [
    `🏅 **${ROLE_LABEL_KO[active]}** 티어·등급 (위 탭에서 역할 전환)`,
    tierLines(p),
  ].join("\n");
  return {
    type: callbackType,
